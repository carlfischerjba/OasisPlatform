...
celery --app src.model_execution_worker.distributed_tasks worker $WORKER_CONCURRENCY -P threads --loglevel=INFO -Q "${OASIS_MODEL_SUPPLIER_ID}-${OASIS_MODEL_ID}-${OASIS_MODEL_VERSION_ID}" |& tee -a /var/log/oasis/worker.log
